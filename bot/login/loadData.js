...
module.exports = async function (api, createLine) {
	...
	// Initialize human behavior mode from saved settings
	if (api) {
		try {
			const behaviorSettings = await globalData.get('humanBehavior', 'data', { enabled: false });
			if (behaviorSettings.enabled) {
				setTimeout(() => {
					global.GoatBot.fcaApi?.setHumanMode?.(true);
					log.info('HUMAN BEHAVIOR', 'Human mode enabled from saved settings');
				}, 5000);
			}
		} catch (err) {
			log.warn('HUMAN BEHAVIOR', 'Could not load behavior settings:', err);
		}
	}
